...
docker run -d \
  ...
  -p 6379:6379 \
  -e TZ=Asia/Seoul \
  -v /dockerProjects/onelife_redis/volumes/data:/data \
...
docker run -d \
  --name onelife_mysql \
  --restart unless-stopped \
  -v /dockerProjects/onelife_mysql/volumes/var/lib/mysql:/var/lib/mysql \
  -v /dockerProjects/onelife_mysql/volumes/etc/mysql/conf.d:/etc/mysql/conf.d \
  --network common \
  -p 3306:3306 \
...
until docker exec onelife_mysql mysql -uroot -p${var.password_1} -e "SELECT 1" &> /dev/null; do
  echo "MySQL이 아직 준비되지 않음. 5초 후 재시도..."
  sleep 5
done
echo "MySQL이 준비됨. 초기화 스크립트 실행 중..."

docker exec onelife_mysql mysql -uroot -p${var.password_1} -e "
